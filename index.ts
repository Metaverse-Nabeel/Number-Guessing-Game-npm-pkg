...
const guessing = (guess: number): { result: boolean; value: number } => {
...
while (wantToPlay) {
  ...
  if (play.playGame === "Yes") {
    ...
    let answer: { result: boolean; value: number } = guessing(game.Num);
    if (answer.result) {
      console.log(chalk.green("**** CONGRATULATIONS!!! ****"));
      console.log(
        chalk.green("The number was: " + chalk.blueBright.bold(answer.value))
      );
      console.log(chalk.green("**** YOU WON! ****"));
    } else {
      console.log(chalk.red("**** BAD LUCK!!! ****"));
      console.log(
        chalk.red("The number was: " + chalk.redBright.bold(answer.value))
      );
